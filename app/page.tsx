...
interface PlanProps {
  ...
  price: string;
  oldPrice?: string;
  benefits?: string[];        // ← agora opcional para maior segurança
  buttonText: string;
  link: string;
  featured?: boolean;
  badge?: string;
...
function PlanCard({
  ...
  // Limpa o preço para enviar ao Facebook Pixel
  ...
  const handleTrack = (e: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    // Evento padrão (importante para o Meta Ads)
    ...
    // Evento customizado para sua análise
      ...
      <ul className="space-y-3 mb-6">
        {(benefits || []).map((benefit, index) => (
          <li key={index} className="flex items-start gap-2">
            <CheckCircle 
              className={`w-5 h-5 flex-shrink-0 mt-0.5 ${featured ? "text-[#00ff88]" : "text-[#00ff88]/70"}`} 
            />
