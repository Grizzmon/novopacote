...
import Script from 'next/script'; // Importante para carregar o Pixel no Next.js
import {
  Check,
  Zap,
  Shield,
...
function PlanCard({ name, price, oldPrice, benefits = [], buttonText, link, featured = false, badge, extraText }: PlanProps) {
  ...
  const handleTrack = () => {
    if (typeof window !== "undefined" && (window as any).fbq) {
      ...
      // Evento InitiateCheckout (Padrão)
  ...
  return (
    <div className={`relative rounded-2xl p-6 transition-all duration-300 ${featured ? "bg-gradient-to-b from-[#0a2a1a] to-[#0f0f0f] border-2 border-[#00ff88] shadow-lg shadow-[#00ff88]/20" : "bg-gradient-to-b from-[#1a1a1a] to-[#0f0f0f] border border-[#00ff88]/40 hover:border-[#00ff88]/60"}`}>
      {badge && (
      ...
      <div className="text-center mb-4 pt-2">
        <h3 className={`text-xl font-bold mb-2 ${featured ? "text-[#00ff88]" : "text-[#00ff88]/80"}`}>{name}</h3>
        <div className="flex items-center justify-center gap-2">
      ...
      <a href={link} target="_blank" rel="noopener noreferrer" className="block" onClick={handleTrack}>
        <Button className={`w-full py-6 text-lg font-bold rounded-xl transition-all duration-300 ${featured ? "bg-[#00ff88] hover:bg-[#00cc6a] text-black shadow-lg shadow-[#00ff88]/30 hover:scale-[1.02]" : "bg-[#00ff88]/20 hover:bg-[#00ff88]/30 text-[#00ff88] border border-[#00ff88]/50 hover:border-[#00ff88] hover:scale-[1.02]"}`}>
          {buttonText}
...
export default function LandingPage() {
  ...
  return (
    <>
      {/* CÓDIGO BASE DO PIXEL INTEGRADO */}
      ...
      <main className="min-h-screen bg-[#0a0a0a] text-white">
        ...
        <section className="relative px-4 pt-12 pb-16 overflow-hidden">
          ...
          <div className="relative max-w-md mx-auto text-center">
            ...
            <h2 className="text-2xl md:text-3xl font-bold text-white mb-4 leading-tight">
              Ative seu Bank Pix e comece a <span className="text-[#00ff88]">receber e sacar dinheiro</span> hoje mesmo
            </h2>
            <p className="text-gray-400 mb-6">Sistema rápido, funcional e pronto para uso — sem complicação</p>
            <div className="mb-6">
            ...
            <Button onClick={scrollToPlans} className="w-full py-6 text-lg font-bold rounded-xl bg-[#00ff88] hover:bg-[#00cc6a] text-black shadow-lg shadow-[#00ff88]/30 transition-all duration-300 hover:scale-[1.02]">
        ...
        <section className="px-4 py-12 bg-[#0f0f0f]">
          <div className="max-w-md mx-auto">
            <h3 className="text-xl font-bold text-center mb-8 text-white">Por que escolher o <span className="text-[#00ff88]">Bank Pix</span>?</h3>
            <div className="space-y-4">
              {[{ icon: Zap, text: "Receba dinheiro rapidamente" }, { icon: Wallet, text: "Saque via M-Pesa e E-mola" }, { icon: CheckCircle, text: "Sistema pronto e fácil de usar" }, { icon: Shield, text: "Sem complicações técnicas" }, { icon: Banknote, text: "Acesso imediato após pagamento" }].map((item, index) => (
                <div key={index} className="flex items-center gap-4 p-4 rounded-xl bg-[#1a1a1a] border border-[#222]">
        ...
        <section ref={plansRef} className="px-4 py-12 bg-[#0f0f0f]">
          <div className="max-w-md mx-auto">
            ...
            <div className="space-y-6">
              <PlanCard name="Bank Pix Pro Max" price="729 MT" oldPrice="1479 MT" badge="MAIS ESCOLHIDO" featured={true} benefits={["Até 10 chaves diferentes", "Saque até 50.000 MT por dia", "Conta protegida e estável", "Uso definitivo", "Receba e saque rápido", "Funcionalidades completas"]} extraText="A maioria dos usuários escolhe este plano" buttonText="Garantir acesso agora" link="https://pay.kambafy.com/checkout/e8ab6f89-80dc-49c1-b937-c19c3a704ba8" />
              <PlanCard name="Bank Pix Start" price="319 MT" benefits={["Apenas 1 chave", "Saque até 500 MT", "Acesso básico", "Funcionalidades limitadas"]} buttonText="Começar com o básico" link="https://pay.kambafy.com/checkout/8e10e34b-8134-4456-9dce-efb470f3dd9b" />
              <PlanCard name="Bank Pix Ultra" price="1099 MT" benefits={["Chaves ilimitadas", "Sem limite de saque", "Tudo do Pro Max incluído", "Prioridade no suporte", "Máxima performance"]} buttonText="Quero acesso completo" link="https://pay.kambafy.com/checkout/7ee14786-af98-49e1-a3b9-272d5857c3d0" />
            </div>
          </div>
        </section>
